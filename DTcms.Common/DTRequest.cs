...
namespace DTcms.Common
{
	...
	public class DTRequest
	{
        ...
        public static string GetRouteDataString(System.Web.Mvc.HtmlHelper html, string name)
        {
            string result = string.Empty;
            if (html.ViewContext.RouteData.Values[name] != null)
            {
                result = html.ViewContext.RouteData.Values[name].ToString();
            }
            return result;
        }
        public static int GetRouteDataInt(System.Web.Mvc.HtmlHelper html, string name)
        {
            int result = 0;
            if (html.ViewContext.RouteData.Values[name] != null)
            {
                Regex obj = new Regex("\\d+");
                Match objmach = obj.Match(html.ViewContext.RouteData.Values[name].ToString());
                if (objmach.Success)
                    result=Convert.ToInt32(objmach.Value);
            }
            return result;
        }
		/// <summary>

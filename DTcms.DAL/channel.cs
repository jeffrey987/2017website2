            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        //删除旧视图重建新视图
                        //RehabChannelViews(conn, trans, model, oldModel.name); 2016.07.20 先关闭此功能
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        strSql1.Append("if exists (select 1 from sysobjects where id = object_id('v_" + channel_name + "') and type = 'V')");
                        strSql1.Append("drop view v_" + channel_name);
